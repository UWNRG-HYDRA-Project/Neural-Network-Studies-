...
class Network:
    def __init__(self, neuronNums):
        ...
        self.weights = [np.random.randn(y,x) for x,y in zip(neuronNums[:-1], neuronNums[1:])]
    ...
    def stochastic_gradient_descent(self, training_data, mini_batch_size, eta, epoches, test_data=None):
        ...
        for j in range(epoches):
            ...
            if test_data is not None:
                print("Epoch {0}: {1} correct IDs / {2} samples".format(j,self.evaluate(test_data), n_tests))
    ...
    def update_weights_and_biases(self, mini_batch, eta):
        ...
        for x, y in mini_batch:
            ...
            del_w = [single_layer_del_w + single_layer_delta_del_w for single_layer_del_w, single_layer_delta_del_w in zip(del_w, delta_del_w)]
    ...
    def backpropogation(self, x, y):
            ...
            output_error = self.cost_prime(activation_list[-1], y)*sigmoid_derivative(z_vectors[-1])
            ...
            for i in range(2, self.neuronLayers):
                ...
                output_error = np.dot(self.weights[-i + 1].transpose(), output_error) * sd
                del_b[-i] = output_error
                del_w[-i] = np.dot(output_error, activation_list[-i - 1].transpose())
            return(del_b, del_w)
